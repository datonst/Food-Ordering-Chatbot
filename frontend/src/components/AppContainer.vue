<template>
  <nav-bar 
    ...
    @login-state-change="handleLoginStateChange"
    @show-restaurants="showRestaurants"
    @show-home="$emit('show-home')"
  ></nav-bar>
...
<script>
  ...
  import HomePage from "./Home/HomeContainer.vue";
      ...
      handleOrderSubmit() {
       ...
       handleNavigation(page) {
         ...
         if (page === 'home') {
           ...
           // Đóng restaurant view nếu đang mở
           if (this.$refs.restaurantsContainer) {
             this.$refs.restaurantsContainer.deselectRestaurant();
           }
       ...
       handleLoginStateChange(loggedIn) {
         ...
         if (loggedIn) {
           this.isChatOpen = true; // Open chat when user logs in
         }
       },
       showRestaurants() {
         this.currentPage = 'restaurants';
         this.showRestaurantsView = true;
         // Trigger restaurants loading if needed
         if (this.$refs.restaurantsContainer) {
           this.$refs.restaurantsContainer.getRestaurants();
         }
       },
       handleShowHome() {
         console.log('AppContainer received show-home event');
         this.$emit('show-home');
       }
     },
     emits: ['show-home'],
...
.main-container {
  width: 100%;
  min-height: calc(100vh - 60px); /* Trừ đi chiều cao của navbar */
  padding: 0;
  margin: 0;
}
